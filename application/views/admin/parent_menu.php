<div id="parentMenuDelete" style="display: none" >
    <div id="delet"></div>
    <div id="yesNo" class="modal-content">
        <div class="delete-box">
            <p>Are you sure?</p>
            <button class="parentMenuYes Yes btn btn-default">Yes</button>
            <button id="parentMenuNo" class="No btn btn-default">No</button>
        </div>
    </div>
</div>
<div class="col-sm-12">
    <section class="right_col">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <table id="example" class="table table-striped responsive-utilities jambo_table">
                <thead class="sortable" data-table="menu">

                <?php foreach($selectparent_menu as $menu):?>
                    <tr class="headings" data-id="<?=$menu["id"]?>">
                        <th class="text_center"><?php echo $menu['arm_title'];?></th>
                        <th class="text_center hidden">
                            <a href="<?php echo base_url('SiteEdit/parentMenu/'.$menu['id']);?>" >
                                разделы
                            </a>
                        </th>
                        <th class="text_center hidden">
                            <img width="70px" height="50px" src="<?php echo base_url('public/images/picture/menu/'.$menu['menuImg']);?>" style="background: transparent" alt=""/>
                        </th>
                        
                        <th class="text_center">
                            <a href="<?php echo base_url('SiteEdit/insertParentSlide/'.$menu['id']);?>">
                                Gallery
                            </a>
                        </th>
                        
                        
                        <th class="text_center hidden">
                            <a href="<?php echo base_url('SiteEdit/parentVideo/'.$menu['id']);?>">
                                Videos
                            </a>
                        </th>                            
                        
                        <th class="text_center">
                            <a href="<?php echo base_url('SiteEdit/edit_menu/'.$menu['id']);?>">
                                <i class="fa fa-pencil" aria-hidden="true"></i>
                            </a>
                        </th>
                        <th class="text_center">
                        
                            <img class='delete deleteParentMenu' src='<?php echo base_url('public/images/picture/delete.png')?>' alt='<?php echo $menu['menuImg']?>' title='<?php echo $menu['id']?>' width="30px" height="30px">
                      
                        </th>
                    </tr>

                <?php endforeach;?>
                </thead>
            </table>
        </div>
        <div class="text-right"><button class="open">Add SubMenu</button></div>

        <div class="addmenu">
            <form action="<?php echo base_url('SiteEdit/insertNewMenu/'.$id_menu);?>" method="POST" enctype="multipart/form-data">
            <div class="form-group">
            <ul class="lang-change-admin">
                <li>
                    <span class="arm-click">arm</span>
                </li>
                <li>
                    <span class="rus-click">rus</span>
                </li>
                <li>
                   <span class="eng-click">eng</span> 
                </li>
            </ul>

            <div class="lang-arm">
                <label for="arm_title" class="col-sm-12 control-label">Section Title Armenian</label>
                <input type="text" id="arm_title" name="arm_title" class="form-control"/>

                <label for="arm_text" class="col-sm-12 control-label">Armenian Text(h1 or h2 > Title , p > Text)</label>
                <textarea name="arm_text" id="arm_text" cols="15" rows="5" class="form-control TinyMCE"></textarea>
                <div class="col-xs-12">
                    <div class="title_lenguage">
                        <div class="title_left">
                            <small>
                                Armenian Title
                            </small>
                        </div>
                        <input type="text" placeholder="" name="arm_meta_title" value="">
                    </div>
                </div>
            </div>
            
            <div class="lang-rus">
                <label for="rus_title" class="col-sm-12 control-label">Section Title Russian</label>
                <input type="text" id="rus_title" name="rus_title" class="form-control"/>

                <label for="rus_text" class="col-sm-12 control-label">Russian Text (h1 or h2 > Title , p > Text )</label>
                <textarea name="rus_text" id="rus_text" cols="15" rows="5" class="form-control TinyMCE"></textarea>
                <div class="col-xs-12">
                    <div class="title_lenguage">
                        <div class="title_left">
                            <small>
                                Russian Title
                            </small>
                        </div>
                        <input type="text" placeholder="" name="rus_meta_title" value="">
                    </div>
                </div>
            </div>

            <div class="lang-eng">
                <label for="eng_title" class="col-sm-12 control-label">Section Title English</label>
                <input type="text" id="eng_title" name="eng_title" class="form-control"/>

                <label for="eng_text" class="col-sm-12 control-label">English Text (h1 or h2 > Title , p > Text )</label>
                <textarea name="eng_text" id="eng_text" cols="15" rows="5" class="form-control TinyMCE"></textarea>
                <div class="col-xs-12">
                    <div class="title_lenguage">
                        <div class="title_left">
                            <small>
                                English Title
                            </small>
                        </div>
                        <input type="text" placeholder="" name="eng_meta_title" value="">
                    </div>
                </div>
            </div>
            <div class="title_left">
                <small style="padding-left: 7px;">
                    Meta Description
                </small>
            </div>
            <textarea name="meta_description" class="textarea-menu"></textarea>
            <div class="hidden">
                <input type="file" id="file_upload" name="parent_menuImg">
                
            </div>
                <label for="url" class="col-sm-12 control-label">URL</label>
                <input type="text" id="url" name="url" class="form-control"/>
            


            
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <div class="button_save_send">
                    <input type="reset" class="Yes btn btn-defaul" value="Reset">
                    <input type="submit" class="No btn btn-defaul" name="addParentMenu" value="Save">
                </div>
            </div>
        </form>
            </div>
        </div>

    </section>
</div>
</body>
</html>

